...
describe('validate middleware (Zod) - chặn dữ liệu sai định dạng TRƯỚC KHI chạm DB', () => {
  ...
  it('POST /api/ticket-holds quantity vượt quá 10 -> 400 (chặn scalping ngay từ validate, chưa cần vào Service)', async () => {
    ...
    // Route này đặt authMiddleware TRƯỚC validate() trong khai báo router
    // -> request này sẽ dừng ở 401 (token sai) chứ không tới được bước
    // validate quantity. Đây là ví dụ tốt để hiểu THỨ TỰ middleware quan
    // trọng: request KHÔNG đăng nhập được thì không bao giờ tới lượt
